...
/*
  Props
  - color (string) default: #FFF — Hex color for particles (3 or 6 chars)
  - quantity (number) default: 100 — How many particles to render
  - staticity (number) default: 50 — Higher = less movement near the cursor
  - ease (number) default: 50 — Lower = particles follow mouse more closely
  - class (string) default: "" — Additional classes for the wrapper div
*/
type Props = {
  color?: string;
  quantity?: number;
...
function animate() {
  clearContext();
  if (context.value) {
    // Reset transform to scaled identity at the start of each frame
